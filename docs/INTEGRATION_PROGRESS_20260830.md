...
- Separate demo credentials: customer@vaanigam.demo / CustomerDemo!2026; admin@vaanigam.demo / AdminDemo!2026. Provisioning uses `apps/api/scripts/provision-demo-identities.ts`, refuses production, and does not reset merchant data. Each seeded customer identity uses its own context tenant; generalized customer registration/multiple customers per tenant is not implemented.
