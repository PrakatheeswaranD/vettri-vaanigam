/**
 * Anumati gateway — end-to-end through the real HTTP route.
 *
...
async function postIntent(body: unknown, headers: Record<string, string> = {}) {
  return app.inject({
    ...
    headers: { "x-agent-id": agent.externalAgentId, ...headers },
...
describe("Anumati gateway — protocol mesh", () => {
...
describe("Anumati gateway — mandate is the gate", () => {
  it("declines an intent presented with no mandate at all", async () => {
    const res = await postIntent({ items: [{ id: sku, quantity: 1 }], buyer: {}, totals: { total: priceMinor } });
    expect(res.statusCode).toBe(403);
    expect(res.json().reasonCode).toBe("MANDATE_MISSING");
  });

  it("declines a mandate whose ceiling was raised after signing", async () => {
    const mandate = { ...mandateFor({ maxAmountMinor: 100 }), maxAmountMinor: 99_000_000 };
    const res = await postIntent({
      items: [{ id: sku, quantity: 1 }],
      buyer: {},
      totals: { total: priceMinor },
      anumati_mandate: mandate,
    });
  ...
  it("refuses to spend the same mandate twice", async () => {
    ...
    const body = { items: [{ id: sku, quantity: 1 }], buyer: {}, totals: { total: priceMinor }, anumati_mandate: mandate };

    const first = await postIntent(body);
    expect(first.statusCode).toBe(200);

    const replay = await postIntent(body);
  ...
  it("does not consume a mandate on a declined intent", async () => {
    ...
    const declined = await postIntent({
    ...
    const accepted = await postIntent({
      items: [{ id: sku, quantity: 1 }],
      buyer: {},
      totals: { total: priceMinor },
      anumati_mandate: mandate,
...
describe("Anumati gateway — the merchant's price is the one that counts", () => {
...
describe("Anumati gateway — the step-up the brief demos on stage", () => {
  it("steps an over-ceiling unknown-agent order up to a human instead of declining it", async () => {
    ...
    const res = await app.inject({
      ...
      payload: {
        ...
        buyer: {},
        totals: { total },
        anumati_mandate: stranger.mandate(merchantId, { maxAmountMinor: total + 1 }),
      },
    });
...
describe("Anumati gateway — an approval becomes payable", () => {
  /**
  ...
  it("does not bill order creation to decision latency", async () => {
    ...
    const approve = await postIntent({
      items: [{ id: sku, quantity: 1 }],
      buyer: {},
      totals: { total: priceMinor },
      anumati_mandate: mandateFor(),
...
describe("Anumati gateway — explainability", () => {
