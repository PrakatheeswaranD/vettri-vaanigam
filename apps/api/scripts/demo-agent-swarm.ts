 ...
 *
 * Drives FIVE simulated buyer agents against a running Vettri Vaanigam gateway
...
async function main() {
  console.log("Vettri Vaanigam — five simulated buyer agents against a live gateway");
  console.log(`API: ${API}`);
  ...
  const acp = await submitIntent(merchant.slug, "agent-chatgpt-acp", {
    ...
    vettri_vaanigam_mandate: mandate(merchant.id, "agent-chatgpt-acp", 2_000_000),
  });
  ...
  const ap2 = await submitIntent(merchant.slug, "agent-gemini-ap2", {
    ...
    vettri_vaanigam_mandate: mandate(merchant.id, "agent-gemini-ap2", 2_000_000),
  });
  ...
  const x402 = await submitIntent(merchant.slug, "agent-x402-wallet", {
    ...
    vettri_vaanigam_mandate: mandate(merchant.id, "agent-x402-wallet", 2_000_000),
  });
  ...
  const upsell = await submitIntent(merchant.slug, "agent-negotiation-test", {
    ...
    vettri_vaanigam_mandate: mandate(merchant.id, "agent-negotiation-test", 3_000_000),
  });
  ...
  const declined = await submitIntent(merchant.slug, unknownAgentId, {
    ...
    vettri_vaanigam_mandate: mandate(merchant.id, unknownAgentId, bigTotal + 1),
  });
