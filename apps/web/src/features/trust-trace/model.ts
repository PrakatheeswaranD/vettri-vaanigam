...
const ACTION_GROUP: Record<string, RawGroup> = {
  ...
  // Kept in step with `activity/model.ts` — the two views must never
  // describe the same ledger row as different things, and an event
  // missing here drops out of the trust trace's pipeline entirely.
  AGENT_INTENT_APPROVED: "POLICY",
  AGENT_INTENT_DECLINED: "POLICY",
  AGENT_INTENT_STEPPED_UP: "POLICY",
  AGENT_STEP_UP_APPROVED: "APPROVAL",
  AGENT_CHECKOUT_CREATED: "COMMERCE",
  AGENT_INVENTORY_RESERVATION_RELEASED: "COMMERCE",
  AGENT_EXECUTION_FAILED_SAFE: "COMMERCE",
  BUYER_PURCHASE_PROPOSED: "PROPOSAL",
  BUYER_PURCHASE_AUTHORIZED: "APPROVAL",
  CUSTOMER_NEGOTIATION_AUTO_APPLIED: "POLICY",
  CUSTOMER_NEGOTIATION_PROPOSED: "POLICY",
  CUSTOMER_NEGOTIATION_DECLINED: "POLICY",
  CAMPAIGN_CREATED: "PROPOSAL",
  PROPOSE_OFFER: "PROPOSAL",
  EVALUATE_OFFER: "POLICY",
  CREATE_CHECKOUT: "COMMERCE",
  DISCOVER_PRODUCT: "INTENT",
  PROPOSE_RECOVERY: "RECOVERY",

  // Workflow events only. Catalogue publishes, readiness recalculations
  // and policy edits are deliberately absent: they are real ledger rows
  // (and the activity feed names them) but they are not steps of ONE
  // purchase, and placing them in the pipeline would describe a workflow
  // that did not happen.
  AGENT_STEP_UP_AUTHORIZATION_EXPIRED: "APPROVAL",
  CUSTOMER_NEGOTIATION_APPROVED: "POLICY",
  CUSTOMER_NEGOTIATION_REJECTED: "POLICY",
  DECISION_CREATED: "POLICY",
  CAMPAIGN_ORDER_ATTRIBUTED: "COMMERCE",
  CHECKOUT_EXPIRED_INVENTORY_RESTOCKED: "COMMERCE",
  ORDER_FULFILLED: "COMMERCE",
  RETURN_REQUESTED: "COMMERCE",
  CAMPAIGN_CONVERSION_RECORDED: "PAYMENT",
  CAMPAIGN_PAYMENT_CAPTURED_CONVERSION: "PAYMENT",
