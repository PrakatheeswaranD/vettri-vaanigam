...
export async function completeBuyerCheckout(proposalId: string): Promise<PaymentDTO | null> {
  ...
  return new Promise((resolve, reject) => {
    const instance = openRazorpayCheckout({ key: checkout.keyId, amount: checkout.amountMinor, currency: checkout.currency, order_id: checkout.providerOrderId, name: "Anumati — Razorpay Test Mode", modal: { ondismiss: () => resolve(null) }, handler: (response) => {
